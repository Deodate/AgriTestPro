...
public class ExpenseServiceImpl implements ExpenseService {
    ...
    private MediaFileRepository mediaFileRepository;
     ...
     // Helper method to extract file ID from URL
    private Long extractFileIdFromUrl(String receiptUrl) {
        try {
            String fileIdString = receiptUrl.substring(receiptUrl.lastIndexOf('/') + 1);
            return Long.parseLong(fileIdString);
        } catch (Exception e) {
            log.error("Error extracting file ID from URL: {}", receiptUrl, e);
            throw new IllegalArgumentException("Invalid receipt URL format", e);
        }
    }
  ...
  private ExpenseResponse mapToExpenseResponse(Expense expense) {
        ...
        // Fetch receipt filename if receipt URL exists
       if (expense.getReceiptUrl() != null) {
            try {
                Long fileId = extractFileIdFromUrl(expense.getReceiptUrl());
                Optional<MediaFile> mediaFile = mediaFileRepository.findById(fileId);
                mediaFile.ifPresent(file -> response.setReceiptFileName(file.getFileName()));
            } catch (Exception e) {
                log.warn("Could not extract filename for receipt URL: {}", expense.getReceiptUrl(), e);
            }
        }
        ...
        if (expense.getApprovedBy() != null) {
            ExpenseResponse.UserDto approvedBy = new ExpenseResponse.UserDto();
            approvedBy.setId(expense.getApprovedBy().getId());
            approvedBy.setUsername(expense.getApprovedBy().getUsername());
            approvedBy.setFullName(expense.getApprovedBy().getFullName());
            response.setApprovedBy(approvedBy);
        }
        
        // Map created by user
        if (expense.getCreatedBy() != null) {
            ExpenseResponse.UserDto createdBy = new ExpenseResponse.UserDto();
            createdBy.setId(expense.getCreatedBy().getId());
            createdBy.setUsername(expense.getCreatedBy().getUsername());
            createdBy.setFullName(expense.getCreatedBy().getFullName());
